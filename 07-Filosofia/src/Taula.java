...
public class Taula {
    ...
    public Taula(int numeroFilosofs) {
        for (int i = 0; i < numeroFilosofs; i++) {
            comensals.add(new Filosof(String.format("fil%d", i), rand));
    ...
    public void showTaula() {
        for (Filosof filosof : comensals) {
            System.out.printf("Comensal: %s esq: %d dret: %d%n", filosof.getName(), filosof.getForquillaEsquerra().getNumero(), filosof.getForquillaDreta().getNumero());
